  ...
  */
  ...
  async exploreTo(terminalPassageNames, { maxDepth = 50, progressReport = true } = {}) {
    ...
    while (knotsToExplore.length > 0) {
      ...
      // Catch self-references, links back to the current knot. That's a mistake in this game.
      if (Object.keys(knotBeingExplored.children).includes(knotBeingExplored.passageName)) {
        const path = [knotBeingExplored];
        while (path[0].parent) { path.unshift(path[0].parent); }
        throw new Error(`Found a self-reference at ${knotBeingExplored.passageName}, `
        + `reached by path:\n` + path.map(k => k.passageName).join('\n'));
      }
    ...
    const passages = [];
    for (const passageName of Object.keys(this.knotsByPassageName)) {
      passages.push(`# ${passageName}`);
      for (const knot of this.knotsByPassageName[passageName]) {
        const parentIds = Object.values(knot._parents).reduce((m, n) => m.concat(n), []).map(k => k.knotId).join(', ');
        passages.push(`## Knot ${knot.knotId} (via ${parentIds})`);
        passages.push(knot.text);
      }
    }
    fs.writeFileSync(path.join(__dirname, '..', 'knots.md'), passages.join(`\n\n`));
...
class Knot {
  ...
  static globalKnotId = 1;

  constructor() {
    this.knotId = Knot.globalKnotId
    Knot.globalKnotId++
  }
